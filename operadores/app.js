...
// depois melhora essa fuction




function TaxaDoAmor() {
    const precoProdutoNormal = parseFloat(document.getElementById('precoProdutoNormal').value);
    const taxaJurosDoAmor = parseFloat(document.getElementById('taxaJurosDoAmor').value);

    if (isNaN(precoProdutoNormal) || isNaN(taxaJurosDoAmor)) {
        alert('Por favor, insira valores válidos.');
        return;
    }

    const juros = precoProdutoNormal * taxaJurosDoAmor;
    const valorTotalFazOL = precoProdutoNormal + taxaJurosDoAmor;
